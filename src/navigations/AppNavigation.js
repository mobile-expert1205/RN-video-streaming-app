...
const AppNavigation = () => {
  ...
  const handleDynamicLinkRouting = (url) => {
    ...
    if (isAuthenticated) {
      ...
      dispatch(setSettingInitial('CompetitionVideo', {
        videoId,
        videoType
      }));
    }
    ...
    switch (data.type) {
      case 'profile':
        if (isAuthenticated) {
          ...
          dispatch(setSettingInitial('UserProfile', {
            userId: data.uid,
            showBack: true,
          }));
        }
      ...
      case 'chatroom':
        if (isAuthenticated) {
          ...
          dispatch(setSettingInitial('ChatRoom', {
            currentuser: {
              id: data.id,
              fullname: data.fullname,
              profile: data.profile
      ...
      case 'vote':
        ...
        if (isAuthenticated) {
          ...
          dispatch(setSettingInitial('CompetitionDetails', {
            competition: JSON.parse(data.competition)
          }));
        }
        break;
  ...
  useEffect(async () => {
    ...
    PushNotification.configure({
      ...
      onNotification: function (notification) {
        if (notification.userInteraction) {
          console.log("User click the notification:", notification.data);
    ...
    async function verifiedUser({uid}) {
      ...
      // console.log("uid",uid,"fcmToken",fcmToken);
      if (fcmToken) {
        firestore()
          .collection('users')
          .doc(uid)
