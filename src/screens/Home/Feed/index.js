...
function Feed({
  ...
  const handleVoting = async () => {
    ...
    if (!voted) {
      setVoted(true)
      setVotes(votes + 1)
      console.log('item in voting' , item)
      onVoting(item, index);
    } else {
      setVoted(false)
      setVotes(votes - 1)
      console.log('item in un voting' , item)
  ...
  const handleLoad = async progress => {
    setCountStatus(true);
    console.log(' on handle load', progress)
    setShowVideo(progress?.duration ? true : false)
  ...
  const onError = onErrorData => {
    ...
    console.log('onErrorData', onErrorData, videoURL);
  };
