...
const Comment = ({route, onBack}) => {
  ...
  const videoType =
    type === 'competition' || type === 'myCompetitionVideos'
      ? 'entries'
      : 'videos';
  console.log('videoType...', videoType,comments);
  ...
  useEffect(() => {
          ...
          if (snapshot) {
            ...
            try {
              snapshot.forEach(obj => {
                ...
                let exist = blockusers.filter(function(v) {
                  return v.id == comment_data.user.uid;
                });
                if (exist.length == 0) {
                  list.push({...obj.data(), id: obj.id});
                }
    ...
    loadData();
  }, [item, dispatch, index, videoType, type, user,isFocused]);
  ...
  const handleSaveComment = async () => {
    if (!editcomment) {
      if (commentmsg.trim() != "") {
        ...
        let comment = commentmsg.trim().split(" ");
  ...
  const onLikeComment = async comment => {
    let exist = likelist.filter(function(v) {
      return v.commentid == comment._id;
    });
    if (exist.length == 0) {
      await firestore()
        .collection(videoType)
  ...
  const renderText = (matchingString, matches) => {
    ...
    console.log('@' + indexes[0]?.username)
      ...
      return (
        <TouchableOpacity
          onPress={() => {
            let cmt =
              keyword.length > 0
                ? commentmsg.slice(0, -keyword.length)
                : commentmsg;
