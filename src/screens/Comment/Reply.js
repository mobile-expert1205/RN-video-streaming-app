...
const Reply = ({ route, onBack }) => {
  ...
  const handleSaveComment = async () => {
    if (!editreply) {
      if (commentmsg.trim() != '') {
        ...
        let comment1 = commentmsg.trim().split(" ");

        console.log('comment.length', comment1.length, pseudoUsers);
      ...
      if (commentmsg.trim() != '') {
        ...
        console.log('comment.length', comment1.length);
        for (let a in comment1) {
          if (comment1[a].includes('@')) {
            let userName = comment1[a].slice(1, comment1[a].length);
            let user = pseudoUsers.find(item => item.username == userName);
  ...
  //   );
  // }
  const handleUserProfilePressed = (reply) => {
    dissMissKeyBoard(true, function () {
      setTimeout(() => {
        navigation.navigate('UserProfile', {
