...
import {
  ...
  Overlay, SafeAreaView,
  View,
  VUText,
  VUTouchableOpacity, VUVideo, VUView
...
import {
  getVideosList, insertVideo as insertVideoOnSQL,
  updateSyncedVideo, updateVideo
} from '../../models/queries';
  ...
  useEffect(() => {
    if (startSync) {
      const processVideo = async () => {
        ...
        const watermarkOption = '[2:v]format=rgba,colorchannelmixer=aa=0.7[fg];[fg][0:v]scale2ref=w="iw*25/100":h="ow/mdar"[wm][vid];[vid][wm]overlay=main_w-overlay_w-20:100 [c]';
        console.log('datalagtime...', karaokeVideo);
        console.log('datalagtime...', lagTime,type);
        if (type === 'karaoke') {
          if (karaokeType == true) {
            const mixingOptions = "[0]volume=1[a];[1]volume=0.5[b];[a][b]amix=inputs=2[aout];";
            ...
            RNFFmpeg.executeAsync(
              ...
              console.log('finalMP4Location', finalMP4Location + " " + new Date().toLocaleString())
    ...
    if (item.fullname != null) {
      return (
        <TouchableOpacity onPress={() => {
          let cmt = keyword.length > 0 ? title.slice(0, - keyword.length) : title;
